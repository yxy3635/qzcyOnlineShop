...
?>
<link href="../assets/js/skin/layer.css" rel="stylesheet">
<style>
...
aria-hidden="true" style="display: none;">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<p></p>
			<span class="label label-info">待考试</span> 等待考试开始
			<p></p>
			<span class="label label-warning">平时分</span> 正常进行平时分，每天45min左右
			</div>
...
function display_zt($zt){
	if($zt==1)
		...
		return '<font color=purple>待考试</font>';
	else if($zt==21)
		return '<font color=brown>平时分</font>';
	else
...
<form action="./list.php" method="GET" class="form-inline">
  <div class="form-group">
	...
	<select name="type" class="form-control"><option value="-1">全部状态</option><option value="0">已提交</option><option value="2">正在处理</option><option value="1">已完成</option><option value="3">异常</option><option value="4">已退单</option><option value="20">待考试</option><option value="21">平时分</option></select>
...
echo '<tr>
							<td>
								'.($res['userid']==$userrow['zid']?'<a href="javascript:showOrder('.$res['id'].',\''.md5($res['id'].SYS_KEY.$res['id']).'\')" title="查看订单详细" class="btn btn-info btn-xs">订单详情</a>':'<a href="javascript:;" class="btn btn-info btn-xs" disabled title="不是你支付的订单">订单详情</a>').'
								<br/><div style="height:4px"></div>
								'.($res['']==$userrow['']?'<a  href="../order/list.php?tid='.$res['tid'].'&user='.$res['input'].'&ptname='.$res['name'].'" title="查看订单进度" class="btn   btn-xs btn-primary">进度详情</a>':'<a href="javascript:;" class="btn   btn-primary btn-xs" disabled title="进度详情">进度详情</a>').'
								<br/><div style="height:4px"></div>	
									'.($res['userid']==$userrow['zid']?'<a href="./workorder.php?my=add&orderid='.$res['id'].'&skey='.md5($res['id'].SYS_KEY.$res['id']).'" title="订单反馈" class="btn btn-info btn-xs" style="background-color:#393D49;">订单反馈</a>':'<a href="javascript:;" class="btn btn-info btn-xs" disabled title="不是你支付的订单" style="background-color:#393D49;">订单反馈</a>').'
							</td>
							<td>
								'.$res['id'].'
							</td>
...
<script src="<?php echo $cdnserver?>assets/js/bootstrap-datepicker.min.js"></script>
<script src="<?php echo $cdnserver?>assets/js/bootstrap-datepicker.zh-CN.min.js"></script>
<script>
function showOrder(id,skey){
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	var status = ['<span class="label label-primary">已提交</span>','<span class="label label-success">已完成</span>','<span class="label label-warning">处理中</span>','<span class="label label-danger">异常</span>','<font color=red>已退款</font>','','','','','','','','','','','','','','','','<span class="label label-info">待考试</span>','<span class="label label-dark">平时分</span>'];
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			...
			if(data.code == 0){
                ...
                // 使用全新 UModal 替代 layer
                UModal.open({
                  title: '订单详细信息',
                  html: item,
                  width: ($(window).width() > 520 ? '520px' : '96vw'),
                  buttons: [
