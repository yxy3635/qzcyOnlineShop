...
switch($act){
case 'neishi_exam_submit':
	$student_id = trim(daddslashes($_POST['student_id']));
	$student_pwd = trim(daddslashes($_POST['student_pwd']));
	if($student_id=='' || $student_pwd=='') exit('{"code":-1,"msg":"学号和密码不能为空"}');
	if(strlen($student_id) < 4) exit('{"code":-1,"msg":"学号格式不正确"}');
	$task_no = date("YmdHis").rand(1000,9999);
	$dir = ROOT.'assets/tmp/neishi_exam/';
	...
	if(file_put_contents($dir.$task_no.'.json', json_encode($payload, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES))){
		exit('{"code":0,"msg":"提交成功","task_no":"'.$task_no.'"}');
	}else{
		exit('{"code":-1,"msg":"服务器写入失败，请检查目录权限"}');
	}
break;
case 'neishi_exam_query':
	$task_no = trim(daddslashes($_GET['task_no']));
	$dir = ROOT.'assets/tmp/neishi_exam/';
	$file = $dir.$task_no.'.json';
	if(!$task_no || !preg_match('/^[0-9]{18}$/', $task_no)) exit('{"code":-1,"msg":"任务号格式不正确"}');
	if(is_file($file)){
		$data = file_get_contents($file);
		exit('{"code":0,"msg":"succ","data":'.$data.'}');
	}else{
		exit('{"code":-1,"msg":"未找到任务号"}');
	}
break;
case 'setpwd':
