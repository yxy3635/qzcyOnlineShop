...
function changepwd(id,skey) {
	pwdlayer = layer.open({
	  ...
	  content: '<div class="form-group"><div class="input-group"><div class="input-group-addon">密码</div><input type="text" id="pwd" value="" class="form-control" placeholder="请填写新的密�? required/></div></div><input type="submit" id="save" onclick="saveOrderPwd('+id+',\''+skey+'\')" class="btn btn-primary btn-block" value="保存">'
	});
}
...
function getPoint() {
	...
	if(multi==1 && count>1){
		$('#need').val('�?+price +"�?�?"+count+"�?);
	}else{
		$('#need').val('�?+price +"�?);
	}
	if(close == 1){
		$('#submit_buy').val('当前商品已停止下�?);
		$('#submit_buy').html('当前商品已停止下�?);
	...
	if($("#inputname2").html() == '说说ID'||$("#inputname2").html() == '说说ＩＤ'||$("#inputname2").attr('gettype')=='ssid'){
		...
		$('#inputvalue').attr("placeholder", "在此输入分享链接 可自动获�?);
		...
		$('#inputvalue').attr("placeholder", "在此输入复制后的链接 可自动转�?);
		$('#inputname').attr("gettype", "shareurl");
	}else if(inputnametype=='pinduoduo'){
		$('#inputvalue').attr("placeholder", "在此粘贴你的拼多多助力口�?);
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_shuoshuo').length > 0){
					if(km==1){
						$('#show_shuoshuo').html('<div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#km_inputvalue\').val(),'+km+','+lastpage+')"><i class="layui-icon layui-icon-prev"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#km_inputvalue\').val(),'+km+','+nextpage+')"><i class="layui-icon layui-icon-next"></i></div></div>');
					}else{
						$('#show_shuoshuo').html('<div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="layui-icon layui-icon-prev"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="layui-icon layui-icon-next"></i></div></div>');
					}
				}else{
					if(km==1){
						$('#km_inputsname').append('<div class="form-group" id="show_shuoshuo"><div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#km_inputvalue\').val(),'+km+','+lastpage+')"><i class="layui-icon layui-icon-prev"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#km_inputvalue\').val(),'+km+','+nextpage+')"><i class="layui-icon layui-icon-next"></i></div></div></div>');
					}else{
						$('#inputsname').append('<div class="form-group" id="show_shuoshuo"><div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="layui-icon layui-icon-prev"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="layui-icon layui-icon-next"></i></div></div></div>');
					}
				}
				set_shuoshuo(id);
			}else{
...
function getsharelink(){
	...
	try{
		...
		if(songid != $("#inputvalue").val())layer.msg('链接转换成功！下单即�?);
	}catch(e){
...
function getpddinput() {
    ...
    var pattresult1 = (/(^[a-zA-Z0-9][a-zA-Z0-9]+)�?/).exec(pddinput);
...
function getmulti(obj){
	...
	price = price * i * mult;
	count = count * i;
	if(count>1)$('#need').val('�?+price.toFixed(2) +"�?�?"+count+"�?);
	else $('#need').val('�?+price.toFixed(2) +"�?);
}
	...
	$.ajax({
		...
		success : function(data) {
			if(data.code == 0){
				...
				$.each(data.data, function(i, item){
					if(item.status==1)
						status='<span class="label label-success">已完�?/span>';
					else if(item.status==2)
						status='<span class="label label-warning">处理�?/span>';
						...
						status='<font color=red>已退�?/font>';
					else
						status='<span class="label label-primary">已提�?/span>';
				...
				var addstr = '';
				if(data.islast==true) addstr += '<button class="btn btn-primary btn-xs pull-left" onclick="queryOrder(\''+data.type+'\',\''+data.content+'\','+(data.page-1)+')">上一�?/button>';
				if(data.isnext==true) addstr += '<button class="btn btn-primary btn-xs pull-right" onclick="queryOrder(\''+data.type+'\',\''+data.content+'\','+(data.page+1)+')">下一�?/button>';
				...
				if($(window).width() > 768){
					...
					layer.open({
					  ...
					  title: '查询订单',
					  skin: 'layui-layer-rim',
					  content: '<div class="table-responsive"><table class="table table-vcenter table-condensed table-striped"><thead><tr><th>下单账号</th><th>商品名称</th><th>数量</th><th class="hidden-xs">购买时间</th><th>状�?/th><th>操作</th></tr></thead><tbody id="list2">'+$('#list').html()+'</tbody></table></div>'
					});
					}
				}else{
					$("#result2").slideDown();
...
function showOrder(id,skey){
	var ii = layer.load(0, {shade:[0.1,'#fff']});
	var status = ['<span class="label label-primary">已提�?/span>','<span class="label label-success">已完�?/span>','<span class="label label-warning">处理�?/span>','<span class="label label-danger">异常</span>','<font color=red>已退�?/font>'];
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>订单基本信息</b></td></tr><tr><td class="info orderTitle">订单编号</td><td colspan="5" class="orderContent">'+id+'</td></tr><tr><td class="info orderTitle">商品名称</td><td colspan="5" class="orderContent">'+data.name+'</td></tr><tr><td class="info orderTitle">订单金额</td><td colspan="5" class="orderContent">'+data.money+'�?/td></tr><tr><td class="info orderTitle">购买时间</td><td colspan="5">'+data.date+'</td></tr><tr><td class="info orderTitle">下单信息</td><td colspan="5" class="orderContent">'+data.inputs+'</td><tr><td class="info orderTitle">订单状�?/td><td colspan="5" class="orderContent">'+status[data.status]+'</td></tr>';
				if(data.complain){
				...
				if(data.list && typeof data.list === "object"){
					if(typeof data.list.order_state !== "undefined" && data.list.order_state && typeof data.list.now_num !== "undefined"){
						...
						item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>订单实时状�?/b></td>';
						$.each(data.list, function(i, v){
							item += '<tr><td class="warning orderTitle">'+i+'</td><td class="orderContent">'+v+'</td></tr>';
						});
					}
				}else if(data.kminfo){
					item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>以下是你的卡密信�?/b></td><tr><td colspan="6" class="orderContent">'+data.kminfo+'</td></tr>';
				}else if(data.result){
				...
				if(data.desc){
					item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>商品简�?/b></td><tr><td colspan="6" class="orderContent">'+data.desc+'</td></tr>';
				}
				item += '</table>';
...
function apply_refund(id,skey){
	var confirmobj = layer.confirm('异常状态订单可以申请退款，退款之后资金会退到用户余额，是否确认退款？', {
	  btn: ['确认退�?,'取消']
	}, function(){
		var ii = layer.load(0, {shade:[0.1,'#fff']});
		$.ajax({
			...
			success : function(data) {
				layer.close(ii);
				if(data.code == 0){
					layer.alert('成功退�?+data.money+'元到余额�?, {icon:1}, function(){ window.location.reload(); });
				}else{
...
function dopay(type,orderid){
	if(type == 'rmb'){
		var ii = layer.msg('正在提交订单请稍�?..', {icon: 16,shade: 0.5,time: 15000});
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(data.code == 0){
					...
					if(data.pay_alipay>0){
						paymsg+='<button class="layui-btn layui-btn-primary layui-btn-fluid" onclick="dopay(\'alipay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="assets/img/alipay.png" class="paylogo">支付�?/button><br/>';
					}
					if(data.pay_qqpay>0){
					...
					if(data.type == 1){
						layer.open({
						  ...
						  skin: 'layui-layer-rim',
						  area: ['320px', '100px'],
						  content: '<div id="captcha"><div id="captcha_text">正在加载验证�?/div><div id="captcha_wait"><div class="loading"><div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div></div></div></div>',
						  success: function(){
						...
						layer.open({
						  ...
						  skin: 'layui-layer-rim',
						  area: ['320px', '260px'],
						  content: '<div id="captcha" style="margin: auto;"><div id="captcha_text">正在加载验证�?/div></div>',
						  success: function(){
						...
						layer.open({
						  ...
						  skin: 'layui-layer-rim',
						  area: ['320px', '231px'],
						  content: '<div id="captcha"><div id="captcha_text">正在加载验证�?/div></div>',
						  success: function(){
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.code == 0){
					...
					$('#cart_count').html(data.cart_count);
					$('#alert_cart').slideDown();
					layer.msg('添加至购物车成功~点击下方进入购物车列表结�?);
				}else if(data.code == 3){
					layer.alert(data.msg, {
						closeBtn: false
					}, function(){
