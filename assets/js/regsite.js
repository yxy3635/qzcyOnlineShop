	...
	$("#submit_buy").click(function(){
		...
		if($("input[name='user']").length>0){
			...
			if(qq.length<5){
				...
				layer.alert('用户名太�?); return false;
			}else if(user.length>20){
				layer.alert('用户名太�?); return false;
			}else if(pwd.length<6){
				layer.alert('密码不能低于6�?); return false;
			...
			success : function(data) {
				...
				if(data.code == 0){
					...
					if(data.pay_alipay>0){
						paymsg+='<button class="btn btn-default btn-block" onclick="dopay(\'alipay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="../assets/img/alipay.png" class="logo">支付�?/button>';
					}
					if(data.pay_qqpay>0){
					...
					if(data.type == 1){
                        layer.open({
                          ...
                          area: ['360px', '140px'],
                          content: '<div id="captcha" style="padding:10px 12px;"><div id="captcha_text">正在加载验证...</div><div id="captcha_wait" style="display:none;"><div class="loading"><div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div></div></div></div>',
						  success: function(){
                        ...
                        layer.open({
						  ...
						  skin: 'layui-layer-rim',
                          area: ['360px', '260px'],
                          content: '<div id="captcha" style="margin:auto; padding:12px;"><div id="captcha_text">正在加载验证...</div></div>',
						  success: function(){
                        ...
                        layer.open({
						  ...
						  skin: 'layui-layer-rim',
                          area: ['360px', '240px'],
                          content: '<div id="captcha" style="padding:12px;"><div id="captcha_text">正在加载验证...</div></div>',
						  success: function(){
