...
class Particle {
	...
	update() {
		...
		// 如果鼠标存在，则检查与鼠标的距�?		if (mouse.x !== null && mouse.y !== null) {
			const dx = mouse.x - this.x;
			const dy = mouse.y - this.y;
			const distanceToMouse = Math.sqrt(dx * dx + dy * dy);
			const interactionRadius = 150;
			if (distanceToMouse < interactionRadius) {
				// 根据距离动态放大粒子，并使用动态透明度的热粉�?				let scaleFactor = (interactionRadius - distanceToMouse) / 30;
	...
	for (let a = 0; a < particlesArray.length; a++) {
		for (let b = a + 1; b < particlesArray.length; b++) {
			...
			if (distance < connectionDistance) {
				// 透明度根据距离调�?				let opacity = 1 - distance / connectionDistance;
...
// 更新并绘制鼠标拖尾效�?function drawMouseTrail() {
	// 遍历拖尾数组，绘制每个轨迹点
	for (let i = 0; i < mouseTrail.length; i++) {
		let point = mouseTrail[i];
		// 以圆点形式绘制，颜色同样使用热粉�?		ctx.fillStyle = `rgba(255,105,180,${point.alpha})`;
		ctx.beginPath();
		ctx.arc(point.x, point.y, 3, 0, Math.PI * 2);
		ctx.fill();
		// 逐渐降低透明�?		point.alpha -= 0.01;
...
function animateParticles() {
	...
	// 绘制粒子之间的连�?	connectParticles();
	// 绘制鼠标与附近粒子的连线，增强悬停效�?	if (mouse.x !== null && mouse.y !== null) {
