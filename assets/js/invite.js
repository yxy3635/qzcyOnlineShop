		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_shuoshuo').length > 0){
					$('#show_shuoshuo').html('<div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="fa fa-chevron-left"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="fa fa-chevron-right"></i></div></div>');
				}else{
					$('#inputsname').append('<div class="form-group" id="show_shuoshuo"><div class="input-group"><div class="input-group-addon onclick" title="上一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')"><i class="fa fa-chevron-left"></i></div><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select><div class="input-group-addon onclick" title="下一�? onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')"><i class="fa fa-chevron-right"></i></div></div></div>');
				}
				set_shuoshuo(id);
			}else{
        ...
        success: function (data) {
            ...
            if (data.code === 0) {
				layer.open({
				  ...
				  content: '<div class="list-group-item well well-sm">' + data.content + '<center><button class="btn btn-warning btn-sm" data-clipboard-text="'+data.url+'" id="copyurl">一键复制链�?/button>&nbsp;<button class="btn btn-success btn-sm" data-clipboard-text="'+data.content+'" id="copycontent">一键复制广告语</button></center></div>'
				});
            } else {
                layer.alert(data.msg);
            }
				...
				success : function(data) {
					...
					if(data.code == 0){
						...
						if(that.attr('data-type') == '1'){
							...
							$('#alert_invite').html('<strong>当前商品推广规则�?/strong><br/>创建推广订单后即可开始发送给好友，别人访问你的推广链接并购买任意商品，你就可以获�?font color="red">'+res.name+'</font>�?);
						}
					}else{
						layer.alert(data.msg);
					}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.code == 0) {
					$.each(data.data, function(i, item) {
						if(item.status==1){
							var type = '<font color="green">已完�?/font>';
						}else var type = '<font color="#ff6347">进行�?/font>'+(item.plan>0?'['+item.click+'/'+item.plan+']':'');
