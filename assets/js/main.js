...
function getPoint() {
	...
	if(inputsname!=''){
		$.each(inputsname.split('|'), function(i, value) {
			...
			if(value=='说说ID'||value=='说说ＩＤ'||inputsnametype=='ssid')
			...
			else if(value=='课程'||value=='课程名'||value=='课程名称')
				var addstr='<div class="input-group-addon onclick" id="iMeo5ckbutton" onclick="get_hzck(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val(),$(\'#inputvalue2\').val(),$(\'#inputvalue3\').val(),$(\'#inputvalue4\').val(),$(\'#inputvalue5\').val(),\'hzck\')">获取课程</div>';
				...
				var addstr='';
			if (value=='课程名称'||value=='课程名'||value=='课程') {
			    $('#inputsname').append('<div class="form-group"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'</div><input type="text" name="inputvalue'+(i+2)+'" disabled placeholder="输入信息后点击→" id="inputvalue'+(i+2)+'" value="" class="form-control" required/>'+addstr+'</div></div>');
			} else if (value=='课程ID'){
			    var kcid_on = "1";
			    $('#inputsname').append('<div class="form-group"><div class="input-group"><div class="input-group-addon" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'</div><input type="text" name="inputvalue'+(i+2)+'" disabled placeholder="无需填写自动获取" id="inputvalue'+(i+2)+'" value="" class="form-control" required/>'+addstr+'</div></div>');
			} else {
...
function get_hzck(id,account,pwd,school,type,km,page){
	var tid=$("#tid").find("option:selected").text();
	if (account === '' || pwd === '' || school === '') {
		layer.alert('请先填写帐号密码学校，再选择课程！');
		return false;
	}
	var tid = $("#tid").val();
    var hash=hashsalt;
	var ii = layer.msg('查询课程中请稍后...', {icon: 16,shade: 0.5,time: 1000*30});
	$.ajax({
		type : "POST",
		url : "../hzck.php",
		data : {tid:tid,school,account,pwd,hash},
		dataType:"json",
		 success:function(data){
			layer.close(ii);
			if(data.code == 1){
				var addstr='';
				var courseCount = data.data.length;
				$.each(data.data, function(i, kc){
					addstr+='<li class="course-item" onclick="var cb=this.querySelector(\'.checkOne\');cb.checked=!cb.checked;this.classList.toggle(\'selected\',cb.checked);"><input type="checkbox" class="course-checkbox checkOne" name="course" courseid="'+kc.id+'" course="'+kc.name+'" onclick="event.stopPropagation();this.closest(\'.course-item\').classList.toggle(\'selected\',this.checked);"/><span class="course-title">'+kc.name+'</span></li>';	
					});
			layer.open({
				type: 1,
				title: '请选择需要的课程',
				...
				area: ['450px','500px'],
				btn: ['确认选择','取消'],
				content: '<div style="padding:0;"><style>.course-modal{font-family:\'Microsoft YaHei\',Arial,sans-serif;}.course-modal .header{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;padding:15px 20px;margin:-1px -1px 0 -1px;border-radius:4px 4px 0 0;}.course-modal .header h4{margin:0;font-size:16px;font-weight:500;}.course-modal .header .count{font-size:12px;opacity:0.9;margin-top:4px;}.course-modal .content{max-height:350px;overflow-y:auto;padding:15px 0;}.course-modal .course-list{list-style:none;margin:0;padding:0;}.course-modal .course-item{display:flex;align-items:center;padding:12px 20px;border-bottom:1px solid #f0f0f0;transition:all 0.2s ease;cursor:pointer;}.course-modal .course-item:hover{background-color:#f8f9ff;}.course-modal .course-item:last-child{border-bottom:none;}.course-modal .course-checkbox{margin-right:12px;width:18px;height:18px;cursor:pointer;accent-color:#667eea;}.course-modal .course-title{flex:1;color:#333;font-size:14px;line-height:1.4;word-break:break-all;}.course-modal .select-all{background-color:#f8f9fa;border-bottom:2px solid #e9ecef;}.course-modal .select-all .course-title{font-weight:500;color:#495057;}.course-modal .course-item.selected{background-color:#e8f4f8;}.course-modal .course-item.selected .course-title{color:#1976d2;font-weight:500;}</style><div class="course-modal"><div class="header"><h4><i class="fa fa-list-ul"></i>&nbsp;&nbsp;课程选择</h4><div class="count">共 '+courseCount+' 门课程可选</div></div><div class="content"><ul class="course-list"><li class="course-item select-all" onclick="var cb=this.querySelector(\'.checkAll\');cb.checked=!cb.checked;$(cb).trigger(\'change\');"><input type="checkbox" class="course-checkbox checkAll" onclick="event.stopPropagation()"/><span class="course-title">全选/取消全选</span></li>'+addstr+'</ul></div></div></div>',
				success: function(layero, index){
					// 添加全选功能增强
					$(layero).find('.checkAll').change(function(){
						var checked = this.checked;
						$(layero).find('.checkOne').each(function(){
							this.checked = checked;
							$(this).closest('.course-item').toggleClass('selected', checked);
						});
					});
				},
				yes: function(index){
					...
					$('input[name="course"]:checked').each(function(){
						course.push($(this).attr("course"));
						courseid.push($(this).attr("courseid"));
					});
					if(course.length === 0){
						layer.msg('请至少选择一门课程！', {icon: 2});
						return false;
					}
					$('#num').val(course.length);
					$('#num').attr("disabled", "true");
					$('#num_add').hide();
					$('#num_min').hide();
					$('#inputvalue4').val(course.join(","));
					$('#inputvalue5').val(courseid.join(",")).hide();
					$('#inputname5').hide();
					layer.msg('已选择 '+course.length+' 门课程', {icon: 1});
					layer.close(index);
			...
			if(data.code == 0){
				...
				$.each(data.data, function(i, item){
					if(item.status==1)
						...
						status='<span class="label label-info">待考试</span>';
					else if(item.status==21)
						status='<span class="label" style="background-color: #8B4513; color: white; padding: 2px 6px; border-radius: 3px;">平时分</span>';
					else
						status='<span class="label label-primary">已提交</span>';
					$('#list').append('<tr orderid='+item.id+'><td>'+item.input+'</td><td>'+item.name+'</td><td>'+item.value+'</td><td class="hidden-xs">'+item.addtime+'</td><td>'+status+'</td><td><a onclick="showOrder('+item.id+',\''+item.skey+'\')" title="查看订单详细" class="btn btn-info btn-xs">详细</a> <a href="../order/list.php?tid='+item.tid+'&user='+item.input+'&ptname='+item.name+'" title="查看进度详细" class="btn btn-success btn-xs">进度</a></td></tr>');
					if(item.result!=null){
						if(item.status==3){
...
function showOrder(id,skey){
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	var status = ['<span class="label label-primary">已提交</span>','<span class="label label-success">已完成</span>','<span class="label label-warning">处理中</span>','<span class="label label-danger">异常</span>','<font color=red>已退款</font>'];
	$.ajax({
		type : "POST",
