...
function cart_shop_save(id) {
	...
	if(($('#inputname').html()=='下单QQ' || $('#inputname').html()=='QQ账号' || $("#inputname").html() == 'QQ账号') && (inputvalue.length<5 || inputvalue.length>11 || isNaN(inputvalue))){layer.alert('请输入正确的QQ号！');return false;}
			...
			success : function(data) {
				if(data.code == 0){
					...
					if("undefined" != typeof layui){
						...
						if(data.pay_alipay>0){
							paymsg+='<button class="btn btn-default btn-block" onclick="dopay(\'alipay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="assets/img/alipay.png" class="paylogo">支付宝</button>';
						}
						if(data.pay_qqpay>0){
							paymsg+='<button class="btn btn-default btn-block" onclick="dopay(\'qqpay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="assets/img/qqpay.png" class="paylogo">QQ钱包</button>';
						}
						if(data.pay_wxpay>0){
							paymsg+='<button class="btn btn-default btn-block" onclick="dopay(\'wxpay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="assets/img/wxpay.png" class="paylogo">微信支付</button>';
						}
						if (data.pay_rmb>0) {
							paymsg+='<button class="btn btn-default btn-block" onclick="dopay(\'rmb\',\''+data.trade_no+'\')" style="margin-top:10px;"><img src="assets/img/rmb.png" class="paylogo">余额支付<span class="text-muted">（剩'+data.user_rmb+'元）</span></button>';
						}
					}
					layer.open({
						type: 1,
						...
						area: ['350px', 'auto'],
						content: '<div class="pay-dialog">' +
							'<div class="pay-header">' +
							'<div class="pay-amount">￥' + data.need + '</div>' +
							'<div class="pay-title">请选择支付方式</div>' +
							'</div>' +
							'<div class="pay-body">' + paymsg + '</div>' +
							'<div class="pay-footer">' +
							'<button class="btn btn-default btn-block" onclick="cancel(\'' + data.trade_no + '\')">取消支付</button>' +
							'</div>' +
							'</div>',
						success: function(layero, index) {
							...
							style.type = 'text/css';
							style.innerHTML = `
								.pay-dialog { padding: 20px; }
								.pay-header { text-align: center; margin-bottom: 20px; }
								.pay-amount { font-size: 30px; font-weight: bold; color: #ff6b6b; margin-bottom: 10px; }
								.pay-title { font-size: 16px; color: #666; }
								.pay-body { margin-bottom: 20px; }
								.pay-body .layui-btn, .pay-body .btn { 
									border-radius: 4px; 
									margin-bottom: 10px; 
									position: relative;
									overflow: hidden;
								...
								.pay-body .layui-btn:hover, .pay-body .btn:hover { 
