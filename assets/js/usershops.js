	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$("#tid").val(0);
				getPoint();
				if(num==0 && cid!=0)layer.msg('<option value="0">没有搜索到相关商�?/option>', {icon: 2, time: 500});
				else layer.msg('成功搜索�?+num+'个商�?, {icon: 1, time: 1000});
			}else{
				layer.alert(data.msg);
			}
		},
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.code == 0){
					...
					layer.alert('<center><h2>�?'+data.need+'</h2><span class="text-muted">�?'+data.num+' 件商�?/span><hr>'+paymsg+'<hr><a class="btn btn-default btn-block" onclick="cancel(\''+data.trade_no+'\')">取消订单</a></center>',{
						btn:[],
						title:'提交订单成功',
						closeBtn: false
					});
