...
var submitVerity = function (postData, failfunc){
	...
	$.ajax({
		...
		success : function(data) {
			layer.close(ii);
			if(data.code == 0){
				layer.alert('恭喜�?您已经完成验�?br>点击确定进入首页�?,{icon:1,end:function (layero,index) {
...
$(document).ready(function(){
	...
	if(captcha_open == 1){
		...
		layer.open({
		  ...
		  closeBtn: false,
		  area: ['320px', '146px'],
		  content: '<div class="input-group"><span class="input-group-addon" style="padding: 0"><img id="codeimg" src="./user/code.php?r=<?php echo time();?>" height="43" onclick="this.src=\'./user/code.php?r=\'+Math.random();" title="点击更换验证�?></span><input type="text" name="code" class="form-control input-lg" required="required" placeholder="输入验证�?/></div><input type="button" value="提交" onclick="codeSubmit()" id="submit_code" class="btn btn-success btn-block btn-lg"/>'
		});
	}
});
